...
class Sokoban_v2(gym.Env):
    ...
    def _push(self, action):
        ...
        if can_push_box:
            ...
            if self.room_state[new_position[0], new_position[1]] in [1, 2, 6]: 
                if self.room_state[new_position[0], new_position[1]] == 6:
            # Nếu tác nhân di chuyển vào bẫy, quay lại vị trí cũ
                    self.player_position = current_position  # Quay lại vị trí ban đầu
                    return False
            ...
            if self.room_fixed[new_box_position[0], new_box_position[1]] == 2:
                box_type = 3 
            elif self.room_fixed[new_box_position[0], new_box_position[1]] == 6:
                self.room_state[current_position[0], current_position[1]] = 5  
                self.room_state[new_box_position[0], new_box_position[1]] = self.room_fixed[new_box_position[0], new_box_position[1]]  # Quay lại vị trí cũ của hộp
                return False, True  
    ...
    def _calc_reward(self):
        ...
        player_on_trap = (self.room_fixed == 6 ) & (self.room_state == 5)
        if player_on_trap.any(): 
           self.reward_last -= self.trap

        
        player_on_trap_box = (self.room_fixed == 6 ) & (self.room_state == 4)
        if player_on_trap_box.any(): 
           self.reward_last -= self.trap_box
    ...
    def room_to_rgb(self, room, room_structure=None):
        ...
        player_filename = pkg_resources.resource_filename(
            resource_package, "/".join(("surface", "mario.png"))
        )
        ...
        trap_filename = pkg_resources.resource_filename(
            resource_package, "/".join(("surface", "special.bmp"))
        )
        traps = imageio.imread(trap_filename)
